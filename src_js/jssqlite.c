...
static JSFunctionSpec sqlite_methods[] = {
    ...
    {"sqlite3_exec",		x_exec,             1},
    {0}
};


JSBool
js_sqlite_initialize(
    JSContext*      cx,
    JSObject*       glob)
{
    ...
    obj = JS_DefineObject( cx, glob, "SQLITE", &sqlite_class, NULL, 0 );
    if( !sap ) status = JS_FALSE;
//  if( status && !JS_DefineProperties(cx, sap, its_props);
    if( status && !JS_DefineFunctions(cx, obj, sqlite_methods) )
        status = JS_FALSE;
    return status;
}
