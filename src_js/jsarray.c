    ...
    jsval*          rval)
{
    ...
    js_GetLengthProperty(cx, obj, &n);
    chars = JS_malloc( cx, (n+1) * sizeof(*chars) );
    for( i  = 0; i < n; i++ )
    {
        JS_GetElement(cx, obj, i, &v);
        chars[i] = JSVAL_TO_INT(v);
    }
    chars[i] = 0;
    str = js_NewString(cx, chars, n, 0);
    *rval = STRING_TO_JSVAL(str);
    return status;
}
